...
ssh -t -i .dev.pem "ubuntu@$HOST" "cd $KEY/app;sed -i \"s/run assessment_a7_reactjs/run assessment_a7_reactjs_$KEY/\" \"bin/setup.sh\""
ssh -t -i .dev.pem "ubuntu@$HOST" "cd $KEY/app;sed -i \"s/exec assessment_a7_reactjs/exec assessment_a7_reactjs_$KEY/\" \"bin/setup.sh\""
ssh -t -i .dev.pem "ubuntu@$HOST" "cd $KEY/app;sed -i \"s/assessment_a7_reactjs/assessment_a7_reactjs_$KEY/\" \"bin/docker/docker-compose.yml\""
